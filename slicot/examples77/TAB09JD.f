...
*     .. Parameters ..
     ...
     $                   LDD = PMAX, LDDV = PMAX,  LDDW = MMAX )
      ...
      PARAMETER        ( LIW1 = 2*( MMAX + PMAX ),
     $                   LIW2 = ( NVPMAX + NWMMAX ) + NMAX + 6,
     $                   LIW3 = ( 2*NVMAX + PMAX + 2 +
     $                            2*NWMAX + MMAX + 2 ) )
      PARAMETER        ( LIWORK = ( LIW1 + LIW2 + LIW3 ) )
      INTEGER          LDW1, LDW2, LDW3, LDW4, LDWORK
      PARAMETER        ( LDW1 = 2*NVPMAX*( NVPMAX + PMAX ) + PMAX*PMAX +
     $                          ( 2*NVPMAX*NVPMAX +
     $                          ( 11*NVPMAX + 16 + PMAX*NVPMAX ) +
     $                               NVPMAX*NMAX +
     $                               ( NVPMAX*NMAX + NMAX*NMAX +
     $                                 PMAX*NMAX + PMAX*MMAX ) ) )
      PARAMETER        ( LDW2 = 2*NWMMAX*( NWMMAX + MMAX ) + MMAX*MMAX +
     $                          ( 2*NWMMAX*NWMMAX +
     $                          ( 11*NWMMAX + 16 + MMAX*NWMMAX ) +
     $                               NWMMAX*NMAX +
     $                             ( NWMMAX*NMAX + NMAX*NMAX +
     $                                    MMAX*NMAX + PMAX*MMAX ) ) )
      PARAMETER        ( LDW3 = NMAX*( 2*NMAX + ( NMAX + MMAX + PMAX )
     $                                 + 5 ) + ( NMAX*( NMAX + 1 ) )/2 )
      PARAMETER        ( LDW4 = NMAX*( MMAX + PMAX + 2 ) + 2*MMAX*PMAX +
     $                          ( NMAX + MMAX ) +
     $                          ( 3*MMAX + 1 + ( NMAX + MMAX ) + PMAX ))
      PARAMETER        ( LDWORK = ( 1 + LDW1 + LDW2 + LDW3 + LDW4 ) )
*     .. Local Scalars ..

...
*     .. Parameters ..
     ...
     $                   LDCC = PMAX, LDDC = PMAX )
      INTEGER          LIWORK
      PARAMETER        ( LIWORK = ( 2*( NMAX + MMAX + PMAX ) +
     $                            NMAX*NMAX ) )
      INTEGER          MPMX
      PARAMETER        ( MPMX = ( MMAX + PMAX ) )
      INTEGER          LDWORK
      PARAMETER        ( LDWORK = 2*MPMX*( 3*MPMX + 2*NMAX ) +
     $                   ( ( NMAX + MPMX )*( NMAX + MPMX + 6 ) +
     $                   MPMX*( MPMX + ( NMAX + MPMX + 5 ) + 1 ) +
     $                   2*NMAX*( NMAX + 2*MPMX ) +
     $                   ( 4*MPMX*MPMX + ( 2*MPMX + 3*NMAX*NMAX +
     $                   ( 2*NMAX*MPMX + 10*NMAX*NMAX+12*NMAX+5 ) ) +
     $                   MPMX*( 3*NMAX + 3*MPMX +
     $                          ( 2*NMAX + 4*MPMX +
     $                               ( NMAX + MPMX ) ) ) ) ) )
*     .. Local Scalars ..

...
C     Workspace
C
C     IWORK   INTEGER array, dimension (2*N+3)
...
C             IWORK(2*q+2) contains the number s of the 1-by-1 blocks,
C             and IWORK(2*q+3) contains the number t of the 2-by-2
C             blocks, corresponding to unreliable eigenvalues.
C
...
C             1-by-1 diagonal elements of T11 and S11 corresponding to
C             unreliable eigenvalues. (Such an eigenvalue is obtained
C             from -i*sqrt(T11(i,i)/S11(i,i)).) Similarly, DWORK(4+2*s),
C             ..., DWORK(3+2*s+8*t) contain the t groups of pairs of
C             2-by-2 diagonal submatrices of T11 and S11 corresponding
C             to pairs of unreliable eigenvalues. (Such an eigenvalue
C             pair is obtained from -i*sqrt( ev ), where ev are the
C             eigenvalues of (T11(i:i+1,i:i+1),S11(i:i+1,i:i+1)).)
...
C             On exit, if INFO = -21, ZWORK(1) returns the minimum
C             value of LZWORK.
C
C     LZWORK  INTEGER
...
C     .. Local Scalars ..
      ...
      INTEGER            I, I1, IA, IB, IDE, IE, IEV, IFG, IQ, IQ2, IQB,
     $                   IS, IW, IW1, IWRK, J, J1, J2, JM1, JP2, K, L,
...
C     B and A, and save in IWORK pointers to the starting element of
C     each pair corresponding to unrealible eigenvalues.
C

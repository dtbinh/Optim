...
C             eigenvalue, corresponding to a 1-by-1 block, while the
C             absolute value of a negative entry in IWORK is an index to
C             the first eigenvalue in a pair of consecutively stored
C             eigenvalues, corresponding to a 2-by-2 block. A 2-by-2
C             block may have two complex, two real, two purely
C             imaginary, or one real and one purely imaginary
C             eigenvalues.
...
C             if IWORK(i-q) < 0.
C             IWORK(2*q+2) contains the number s of finite eigenvalues
C             corresponding to the 1-by-1 blocks, and IWORK(2*q+3)
C             contains the number t of the 2-by-2 blocks.
C             If INFO = 0, then q = 0.
C
C     LIWORK  INTEGER
C             The dimension of the array IWORK.
C             LIWORK >= MAX(N+12, 2*N+3).
C
...
C     .. Local Scalars ..
     ...
     $                   LUPDQ1, LUPDQ2, TOSAVE
      CHARACTER*16       CMPQ, CMPSC
      INTEGER            EMAX, EMIN, I, I11, I22, IC, ICS, IMAT, IW,
     $                   IWARN, IWRK, J, JA, JB, JC, JE, JS, K, L, M,
     $                   M1, MJ2, MJ3, MK1, MK2, MK3, MM, N1, N2, N3,
     $                   NB, NBETA0, NC, NINF, OPTDW, P
      DOUBLE PRECISION   BASE, CO, MU, NRMS, NU, PREC, SI, TMP1, TMP2
      COMPLEX*16         EIG
C
C     .. Local Arrays ..
      INTEGER            IDUM( 1 )
...
C     .. External Subroutines ..
     ...
     $                   DLARFG, DLARTG, DLASET, DLASSQ, DROT, DSYMV,
     $                   DSYR2, MA02AD, MA02PD, MB01LD, MB01MD, MB01ND,
     $                   MB03BD, XERBLA
C
...
C
     ...
     $         ( LUPDQ2 .AND. .NOT.LUPDQ1 ) ) THEN
      ...
      ELSE IF( LDC2.LT.M1 ) THEN
         INFO = -22
      ELSE IF( LIWORK.LT.MAX( N+12, 2*N+3 ) ) THEN
         INFO = -27
      ELSE
...
C
      NINF = 0
      IF( DLANGE( 'Max', M, M+1, DE, LDDE, DWORK ).EQ.ZERO ) THEN
         IF( M.EQ.1 ) THEN
            NRMS   = ABS( A( 1, 1 ) )
            IF( NRMS.EQ.ZERO )
     $         NINF = 1
   ...
   10          CONTINUE
         ...
         NRMS = NRMS*SQRT( TWO )
      ELSE
         NINF = MA02OD( 'Skew', M, A, LDA, DE, LDDE )/2
         ...
         DO 430  J = 1, NINF - NBETA0
            ...
            DO 420 I = 1, M
               IF( BETA( I ).GT.ZERO ) THEN
                  IF( DLAPY2( ALPHAR( I ), ALPHAI( I ) )*TMP2.GT.
     $                        TMP1*BETA( I ) ) THEN
                     TMP1 = DLAPY2( ALPHAR( I ), ALPHAI( I ) )
                     TMP2 = BETA( I )
                     P    = I
                  END IF
               END IF
...
C
      END IF
C
C     Save information about blocks with possible loss of accuracy.
C
      IF( IWARN.GT.M ) THEN
         I = 2
C
C        Compress part of IWORK corresponding to unreliable eigenvalues.
C
         DO 460  J = M + 6, 2*M + 5
            K = IWORK( J )
            IF( K.NE.0 ) THEN
...
C
         ...
         ELSE
            P = I
C
C           Compress part of IWORK with pointers to the blocks
C           corresponding to unreliable eigenvalues.
C
            DO 470  J = M + 6, 2*M + 5
               K = IWORK( J )
               IF( K.NE.0 ) THEN
...
C
      K = 4*I11 + 16*I22
      TOSAVE = .NOT.( LTRI .OR. LCMPQ1 .OR. LCMPQ2 ) .AND. M.LE.2
      IF ( TOSAVE ) THEN
         CALL DCOPY( 4, DWORK( IWRK ), 1, DUM, 1 )
         CALL DCOPY( K, DWORK( IWRK+4 ), 1, DWORK( 6 ), 1 )
         CALL DCOPY( 4, DUM, 1, DWORK( 2 ), 1 )
      ELSE
         CALL DCOPY( K+4, DWORK( IWRK ), 1, DWORK( 2 ), 1 )
      END IF
C
      DWORK( 1 ) = OPTDW
      RETURN

...
C     .. Local Scalars ..
      ...
      INTEGER           I, I1, II, ITAU, J, JWORK, KABCD, LABCD2,
...
C
     ...
     $              LDABCD )
      CALL DLASET( 'Full', N, N, ZERO, ONE, E, LDE )
C
...
C
C        Extract the reduced the pencil S2(lambda) to
C
C             ( Br  Ar-lambda*I ),
C             ( Dr      Cr      )
C
...
C
     ...
     $                NIZ, NU, MU, I1, NKROR, IWORK, KRONR, TOLER,
...
C
     ...
     $                DWORK(NU1), LDABCD, DWORK(ITAU), DWORK, LDABCD,
     $                DWORK(JWORK), LDWORK-JWORK+1, INFO )
         WRKOPT = MAX( WRKOPT, INT( DWORK(JWORK) ) + JWORK - 1 )
C

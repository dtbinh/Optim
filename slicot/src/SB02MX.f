...
C             If FACT = 'N' and DEF = 'D', the stricly lower triangular
C             part (if UPLO = 'U') or stricly upper triangular part
C             (if UPLO = 'L') of this array is used as workspace (filled
C             in by symmetry). If FACT = 'N' and DEF = 'I', the stricly
...
C     .. Array Arguments ..
     ...
     $                  L(LDL,*), Q(LDQ,*), R(LDR,*)
C     .. Local Scalars ..
      LOGICAL           LDEF, LFACTC, LFACTU, LFLAG, LJOBG, LJOBL,
     $                  LNFACT, LTRANS, LUPLOU
...
C
            CALL DTRSM( 'Right', UPLO, TRANSU, 'Non-unit', N, M, ONE, R,
     $                  LDR, B, LDB )
C
            IF( LJOBG ) THEN
C                                         -1

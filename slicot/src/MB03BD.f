...
C     Jan. 2013, Feb. 2013, July 2013, Sep. 2016, Nov. 2016.
C
C     KEYWORDS
C
...
C     .. Local Scalars ..
     ...
     $                  SVMN, SVMX, TEMP, TEMP2, TEMP3, TOL, TOLL, ULP
...
C     .. External Subroutines ..
      EXTERNAL          DLABAD, DLARNV, DLARTG, DLAS2, DLASET, DROT,
     $                  MA01BD, MB03AD, MB03BA, MB03BB, MB03BC, MB03BE,
     $                  XERBLA
C     .. Intrinsic Functions ..
      INTRINSIC         ABS, DBLE, INT, LOG, MAX, MIN, MOD, SIGN
C
...
C
         IF ( BETA(ILAST).NE.ZERO ) THEN
            DO 400  L = 1, K
               IF ( ABS( A(ILAST,ILAST,L) ).LT.DWORK(L)*TOLL ) THEN
                  IWARN = N + 1
...
C
            DO 460  L = 1, K
               IF ( ALPHAI(J).EQ.ZERO ) THEN
                  IF ( ABS( A(J,J,L) ).LT.DWORK(L)*TOLL ) THEN
                     IWARN = N + 1
                     IWORK(2*K+ILAST) = -ILAST
                     GO TO 470
                  END IF

      SUBROUTINE SG02ND( DICO, JOBE, JOB, JOBX, FACT, UPLO, JOBL,
     $                   TRANS, N, M, P, A, LDA, E, LDE, B, LDB, R, LDR,
     $                   IPIV, L, LDL, X, LDX, RNORM, K, LDK, H, LDH,
     $                   XE, LDXE, OUFACT, IWORK, DWORK, LDWORK, INFO )
C
...
C             non-negative definite if DICO = 'D' and (FACT = 'D' or
C             FACT = 'C').
C             The full matrix X must be given on input if LDWORK < N*M
C             and ((JOBX = 'N' and N >= M) or (JOBX = 'C', DICO = 'D',
C             FACT = 'N' and N < M)).
...
C             If JOB = 'H' or ((JOB = 'F' or JOB = 'D' or JOB = 'C') and
C             OUFACT(1) = 2), the leading N-by-M part of this array
C             contains the matrix H.
C             If (JOB = 'F' or JOB = 'C') and OUFACT(1) = 1, the leading
...
C             LDWORK >= max(2,3*M,a)       if FACT <> 'U', DICO = 'C',
C                                          or FACT =  'N' or
C                                            (FACT =  'C' and (JOB = 'C'
C                                          or JOB  =  'D')), DICO = 'D';
C             LDWORK >= max(N+3*M+2,4*N+1) if FACT =  'D' or
C                                            (FACT =  'C' and JOB <> 'C'
C                                          and JOB <> 'D'), DICO = 'D'.
...
C     .. Array Arguments ..
     ...
     $                  X(LDX,*), XE(LDXE,*)
C     .. Local Scalars ..
      LOGICAL           DISCR, HAVEXE, LFACTA, LFACTC, LFACTD, LFACTU,
     ...
     $                                    .AND. LDXE.LT.N ) ) THEN
         ...
         ELSE
            ...
            IF( LDWORK.EQ.-1 ) THEN
               IF ( M.GT.N ) THEN
                  IF ( WITHXE ) THEN
                     IF ( DISCR .AND. LNFACT ) THEN
                        WRKOPT = NM
                     ELSE
                        WRKOPT = 0
                     END IF
                  ELSE
                     IF ( ( (      DISCR .AND. LFACTA ) .OR.
     $                      ( .NOT.DISCR .AND. LJOBE ) ) ) THEN
                        WRKOPT = N*N
                     ELSE
                        WRKOPT = 0
                     END IF
...
C     in K or XB in the workspace, if enough space. If K is used and for
C     DICO = 'D' and FACT = 'N', compute R+B'XB in R. Then, compute in K
C        B'Xop(A) + L', if DICO = 'D';
C        B'Xop(E) + L', if DICO = 'C'.
C     If the workspace is used, reverse the order of the last two steps.
C     If JOB = 'H', set H = K'. The same formulas are used for JOB = 'D'
C     or JOB = 'C', but B and L differ.
...
C
      ELSE
         IF ( SUFWRK ) THEN
            HAVEXE = .TRUE.
            IF ( DISCR .OR. LJOBE .OR. .NOT.WITHL ) THEN
               CALL DSYMM(  'Left', UPLO, N, M, ONE, X, LDX, B, LDB,
     $                      ZERO, DWORK, N )
               IF ( WITHH ) THEN
                  IF ( WITHL ) THEN
                  ...
                  END IF
                  IF ( DISCR ) THEN
                     CALL DGEMM( NTRANS, NT, N, M, N, ONE, A, LDA,
     $                           DWORK, N, TEMP, H, LDH )
                  ELSE IF ( LJOBE ) THEN
                     CALL DGEMM( NTRANS, NT, N, M, N, ONE, E, LDE,
     $                           DWORK, N, TEMP, H, LDH )
                  ELSE
                     CALL DLACPY( 'All', N, M, DWORK, N, H, LDH )
                  END IF
                  CALL MA02AD( 'All', N, M, H, LDH, K, LDK )
               ELSE IF ( WITHL ) THEN
                  CALL MA02AD( 'All', N, M, L, LDL, K, LDK )
                  IF ( DISCR ) THEN
                     CALL DGEMM( TR, TRANS, M, N, N, ONE, DWORK, N, A,
     $                           LDA, ONE, K, LDK )
                  ELSE
                     CALL DGEMM( TR, TRANS, M, N, N, ONE, DWORK, N, E,
     $                           LDE, ONE, K, LDK )
                  END IF
               ELSE IF ( DISCR ) THEN
...
C
      IF ( .NOT.HAVEXE ) THEN
         MS = MAX( LDWORK/N, 1 )
C
C        Postmultiply B'X by op(A) or by op(E).
...
C
            DO 10 I = 1, M, MS
               ...
               CALL DGEMM(  NT, TRANS, NR, N, N, ONE, DWORK, NR, A, LDA,
     $                      ZERO, K(I,1), LDK )
   10       CONTINUE
C
         ELSE IF ( LJOBE ) THEN
C
            DO 20 I = 1, M, MS
               NR = MIN( MS, M-I+1 )
               CALL DLACPY( 'All', NR, N, K(I,1), LDK, DWORK, NR )
               CALL DGEMM(  NT, TRANS, NR, N, N, ONE, DWORK, NR, E, LDE,
     $                      ZERO, K(I,1), LDK )
   20       CONTINUE
C
         END IF
...
C
         IF( WITHH )
     $      CALL MA02AD( 'All', M, N, K, LDK, H, LDH )
      END IF
C
      WRKOPT = MAX( WRKMIN, WRKOPT )
C
